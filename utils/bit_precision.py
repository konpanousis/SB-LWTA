...
def compute_compression_rate(w_mus, w_vars, masks):
    # reduce architecture
    
    # compute overflow level based on maximum weight
    overflow = np.max([np.max(np.abs(w)) for w in w_mus])
    # compute compression rate
    CR_architecture, CR_fast_inference, _, _ = _compute_compression_rate(w_vars, dist_fun=lambda x: np.mean(x), overflow=overflow)
...
def compute_reduced_weights(w_mus, w_vars):
    overflow = np.max([np.max(np.abs(w)) for w in w_mus])
    CR_architecture, CR_fast_inference, significant_bits, exponent_bits = _compute_compression_rate(w_vars, dist_fun=lambda x: np.mean(x), overflow=overflow)
    ...
    return weights, significant_bits, exponent_bits 